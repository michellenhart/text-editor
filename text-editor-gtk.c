        ...
        if (flag) {
            ...
            switch (msg.type) {
                ...
                case MSG_LINE_GRANTED:
                    ...
                    snprintf(state->status_buf, sizeof(state->status_buf), "Linha %d reservada para edição.", state->editing_line+1);
                    g_idle_add(gui_set_status, state);
                ...
                case MSG_LINE_DENIED:
                    ...
                    break;
                case MSG_CHAT_PRIVATE:
                    snprintf(state->status_buf, sizeof(state->status_buf), "[Privado] %.245s", msg.text);
                    g_idle_add(gui_set_status, state);
                    break;
                default:
                    break;
...
void on_private_msg(GtkButton *button, gpointer user_data) {
    ...
    msg.sender_rank = state->my_rank;
    msg.target_rank = 1 + ((state->my_rank == 1) ? 1 : 0); // envia para o outro cliente, simples exemplo
...
void setup_gui(ClientState *state, int argc, char **argv) {
    ...
    gtk_window_set_title(GTK_WINDOW(state->window), "Editor Colaborativo");
